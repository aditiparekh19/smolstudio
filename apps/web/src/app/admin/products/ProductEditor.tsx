...
function blankVariant(productId: string) {
  return { id: undefined as string | undefined, productId, sku: "", size: "", color: "", stock: 0 };
}
export default function ProductEditor({ id }: { id?: string }) {
  ...
  if (!product)
    return (
      <main className="mx-auto max-w-6xl px-5 py-16">
        {error ? <><p className="rounded-xl bg-red-50 p-4 text-sm text-red-700">{error}</p><button onClick={() => router.refresh()} className="mt-4 rounded-full border border-[#cdbfb5] px-5 py-3 text-sm">Retry</button></> : 'Loading product…'}
      </main>
    );
  const p = product;
  async function uploadFiles(productId: string, files: File[], current: Product) {
    let latest = current;
    for (const file of files) {
      ...
      const response = await apiClient().request<{ uploadProductImage: Product }>(adminUploadImageMutation, {
        productId, filename: file.name, contentType: file.type, dataBase64: base64, altText: latest.name
      });
  ...
  async function save() {
    ...
    setMessage("");
    try {
      const r = await apiClient().request<{ saveAdminProduct: Product }>(adminSaveProductMutation, {
        id: p.id || undefined, name: p.name, slug: p.slug, description: p.description || null,
        priceInr: Number(p.priceInr), compareAtPriceInr: p.compareAtPriceInr == null ? null : Number(p.compareAtPriceInr),
        sku: p.sku, categoryId: p.categoryId, isActive: p.isActive
      });
      ...
      setProduct(saved);
      setMessage(pendingImages.length ? "Product and images saved." : "Product saved.");
  ...
  function queueImages(files: FileList | null) {
    ...
    setMessage(`${selected.length} image${selected.length === 1 ? "" : "s"} ready to save with the product.`);
  }
            ...
            <div className="flex items-center justify-between">
              ...
              <label className="cursor-pointer rounded-full bg-[#5e473c] px-5 py-3 text-sm text-white">
                ...
                <input
                  ...
                  multiple
                  className="hidden"
                  onChange={(e) => { queueImages(e.target.files); e.currentTarget.value = ""; }}
                />
              </label>
            </div>
            {pendingImages.length > 0 && (
              <p className="mt-4 rounded-xl bg-[#fffaf4] p-4 text-sm text-[#75645b]">{pendingImages.length} new image{pendingImages.length === 1 ? "" : "s"} will be uploaded when you click <strong>Save product</strong>.</p>
            )}
            {p.id && (
