...
eligiblePreviousQuantity: number;
freeRemainingQuantity: number;
...
useState<RequestType>("PRODUCT_FAULT");
const [afterSalesReason, setAfterSalesReason] = useState("");
const [requestedSize, setRequestedSize] = useState("");
const [returnQuantity, setReturnQuantity] = useState(1);

const [afterSalesFeePreview, setAfterSalesFeePreview] =
useState<AfterSalesFeePreview | null>(null);
...
async function loadAfterSalesFeePreview() {
  try {
    const result = await apiClient().request<{
      afterSalesFeePreview: AfterSalesFeePreview;
    }>(afterSalesFeePreviewQuery, {
      requestType,
      returnQuantity,
    });
    ...
    if (!cancelled) {
      setAfterSalesFeePreview(result.afterSalesFeePreview);
    }
  } catch {
    if (!cancelled) {
      setAfterSalesFeePreview(null);
...
}, [activeItemId, requestType, returnQuantity]);
...
if (!user) {
return ( <main className="mx-auto max-w-5xl px-5 py-16"> <Link href="/login" className="underline">
Sign in </Link> </main>
);
}

if (!o) {
return ( <main className="mx-auto max-w-5xl px-5 py-16 text-red-700">
{error || "Order not found."} </main>
);
}
...
if (
  ...
  setError(
    `Quantity must be between 1 and ${Number(item.quantity)}.`,
  );
  return;
}
...
try {
  ...
  if (response.requestType === "PRODUCT_FAULT") {
    const fee = Number(response.returnFeeInr ?? 0);

    setSuccess(
      `Product fault request submitted for ${response.returnQuantity} ${
        response.returnQuantity === 1 ? "item" : "items"
      }. Paid amount considered: ₹${Number(
        response.calculatedPaidAmountInr,
      ).toLocaleString("en-IN")}.${
        fee > 0
          ? ` Return fee: ₹${fee.toLocaleString("en-IN")}.`
          : " No return fee."
      }`,
    );
  } else {
    setSuccess(
      `Size replacement request submitted for ${response.returnQuantity} ${
        response.returnQuantity === 1 ? "item" : "items"
      } for ${response.requestedSize}. No return fee.`,
    );
  }
} catch (e) {
  setError(
...
return ( <main className="mx-auto max-w-5xl px-5 py-14 lg:px-8"> <Link href="/account/orders" className="text-sm text-[#8b7a70]">
← Orders </Link>
              ...
              >
                ...
                {existingRequest && (
                  <div className="mt-4 rounded-2xl bg-[#fffaf4] p-4">
                    ...
                    {existingRequest.returnQuantity > 0 && (
                      <p className="mt-2 text-xs text-[#8b7a70]">
                        Quantity:{" "}
                        <strong>{existingRequest.returnQuantity}</strong>
                      </p>
                    )}
                    ...
                    {existingRequest.approvedCreditInr != null && (
                      <div className="mt-2 space-y-1 text-xs text-[#8b7a70]">
                        <p>
                          Store credit: ₹
                          {Number(
                        ...
                        {Number(existingRequest.returnFeeInr ?? 0) > 0 && (
                          <p>
                            Return fee: ₹
                            {Number(
                              existingRequest.returnFeeInr,
                            ).toLocaleString("en-IN")}
                          </p>
                        )}

                        <p className="font-medium text-[#5e473c]">
                          Amount credited: ₹
                          {Math.max(
                            0,
                            Number(existingRequest.approvedCreditInr ?? 0) -
                              Number(existingRequest.returnFeeInr ?? 0),
                          ).toLocaleString("en-IN")}
                        </p>
                      </div>
                    )}
                  ...
                  existingRequest.replacementOrderId && (
                    <div className="mt-4 rounded-2xl border border-[#eadfd5] bg-[#fffaf4] p-4">
                      ...
                      <p className="mt-1 text-xs leading-5 text-[#8b7a70]">
                        Your replacement order has been created and is
                        being processed.
                      </p>
                  ...
                  <div className="mt-4 rounded-2xl border border-[#eadfd5] bg-[#fffaf4] p-5">
                    ...
                    <div className="mt-4">
                      <label
                        ...
                        className="block text-sm font-medium text-[#5e473c]"
                      >
                        Quantity to return/replace
                      </label>
                      ...
                      <select
                        ...
                        disabled={busy}
                        className="mt-2 rounded-xl border border-[#d9cbc0] bg-white px-4 py-3 text-sm text-[#5e473c] outline-none"
                      >
                        {Array.from(
                          {
                            length: Math.max(
                      ...
                      <p className="mt-2 text-xs text-[#8b7a70]">
                        You purchased {Number(item.quantity)}{" "}
                        {Number(item.quantity) === 1 ? "item" : "items"}.
                      </p>
                    </div>

                    {/* Return fee preview */}
                    {afterSalesFeePreview && (
                      <div className="mt-4 rounded-xl border border-[#e5d8ce] bg-[#faf7f4] p-4">
                        {requestType === "SIZE_REPLACEMENT" ? (
                          <p className="text-sm font-medium text-[#5e473c]">
                            No return fee for size replacement.
                          </p>
                        ) : afterSalesFeePreview.returnFeeInr > 0 ? (
                          <>
                            <p className="text-sm font-medium text-[#5e473c]">
                              Return fee: ₹
                              {Number(
                                afterSalesFeePreview.returnFeeInr,
                              ).toLocaleString("en-IN")}
                            </p>

                            <p className="mt-1 text-xs leading-5 text-[#8b7a70]">
                              Your first 2 eligible product-fault return
                              items are free. Any quantity beyond your
                              remaining free allowance is charged at ₹100
                              per item. This fee will be deducted from your
                              approved store credit.
                            </p>
                          </>
                        ) : (
                          <p className="text-sm font-medium text-[#5e473c]">
                            No return fee for this request.
                          </p>
                        )}
                      </div>
                    )}
