...
export default function CartPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-14 lg:px-8">
      ...
      <h1 className="mt-2 font-serif text-5xl tracking-[-0.04em] text-[#5e473c]">
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="mt-10 rounded-4xl border border-[#eadfd5] bg-[#fffaf4] p-10 text-center">
          ...
          <Link
            href="/"
            className="mt-5 inline-block rounded-full bg-[#5e473c] px-6 py-3 text-sm text-white"
          >
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_320px]">
          <div className="space-y-3">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex gap-4 rounded-2xl border border-[#eadfd5] bg-[#fffaf4] p-4"
              >
                <div className="relative h-24 w-20 shrink-0 overflow-hidden rounded-xl bg-[#f1e5d7]">
                  <Image
                    src={
                      item.slug === "sunny-fruit-vest"
                        ? "/products/sunny-fruit-vest.png"
                        : (item.imageUrl ??
                          `https://placehold.co/300x380/F3E7D7/5E473C?text=${encodeURIComponent(item.name)}`)
                    }
                    alt={item.name}
                    fill
                    unoptimized
                ...
                <div className="min-w-0 flex-1">
                  <Link
                    ...
                    className="font-medium text-[#332c28] hover:underline"
                  >
                    {item.name}
                  </Link>
                  ...
                  <p className="mt-1 text-sm text-[#5e473c]">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      ...
                      className="h-8 w-8 rounded-full border border-[#d9cbc0] disabled:opacity-40"
                    >
                      −
                    </button>
                    ...
                    <button
                      onClick={() => void update(item.id, item.quantity + 1)}
                      className="h-8 w-8 rounded-full border border-[#d9cbc0]"
                    >
                      +
                    </button>
                    <button
                      onClick={() => void remove(item.id)}
                      className="ml-3 text-xs text-[#8b7a70] underline"
                    >
                      Remove
                    </button>
                  </div>
          ...
          <aside className="h-fit rounded-[2rem] border border-[#eadfd5] bg-[#fffaf4] p-6">
            ...
            <Link
              href="/checkout"
              className="mt-6 block rounded-full bg-[#5e473c] px-6 py-4 text-center text-sm font-medium text-white"
            >
