...
async function findOrCreateCart(
  ...
  const existing = await pool.request().input("anonymousToken", anonymousToken)
    .query<{ id: string }>(`
  ...
  const id = randomUUID();
  await pool.request().input("id", id).input("anonymousToken", anonymousToken)
    .query(`
...
export async function mergeGuestCart(
  ...
  const guest = await pool.request().input("anonymousToken", anonymousToken)
    .query<{ id: string }>(`
  ...
  try {
    ...
    request
      ...
      .input("customerCartId", customerCartId);
    await request.query(`
...
export async function getCart(
  ...
  const result = await pool.request().input("cartId", cartId).query<
    CartItem & { priceInr: number; totalInr: number }
  >(`
    SELECT
      ci.id,
...
export async function addToCart(
  ...
  try {
    ...
    const variant = await request.query<{
      available: number;
      priceInr: number;
    }>(`
    ...
    if (current + quantity > row.available)
      throw new Error(`Only ${row.available} item(s) are available.`);
...
export async function updateCartItem(
  ...
  const pool = await getDb();
  const availability = await pool.request().input("variantId", item.variantId)
  ...
  `);
  if ((availability.recordset[0]?.available ?? 0) < quantity)
    throw new Error("Requested quantity is not available.");
  await pool.request().input("itemId", itemId).input("quantity", quantity)
    .query(`
...
export async function removeCartItem(
  ...
  const cart = await getCart(customerId, anonymousToken);
  if (!cart.items.some((x) => x.id === itemId))
